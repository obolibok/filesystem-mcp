...
002-tool интегрирована. Уточняем общий контракт 003/004 по пользовательскому опыту
снимка около 500 МБ: фоновые задания, потоковое разбиение, сжатие, хранение и выдача
частей входят уже в 003; 004 использует эту основу для выбранных originals.
[Проектное предложение](snapshot-bundle-design.md) описывает сценарий и проверки.
[Разбор переданного CSV](../testing/snapshot-shape-2026-09-20.md): 2,9 млн записей,
448,51 MB несжатых данных; самостоятельные ZIP-части 1,47–5,53 MB.
Обе задачи остаются proposed; код не назначен. 50 МБ не является подтверждённым
пределом MCP-доставки; до фиксации размеров частей нужен целевой size ladder.
