...
export function buildSectionsRecord(readOnly: boolean): Record<string, string> {
  ...
  return {
    ...
    constraints: [
      ...
      `${SNAPSHOT.name}: Reuses a matching completed snapshot up to maxAgeMs (default one hour from walk start), joins matching queued/running work, or starts a job. Poll ${JOB_STATUS.name}; fetch manifest and ZIP parts with ${GET_ARTIFACT.name}. forceRefresh starts a new scan; use an optional idempotencyKey for safe retries, especially with forceRefresh. A forced retry without a key starts another scan. Shared cancellation affects all observers. Source changes are not detected automatically.`,
